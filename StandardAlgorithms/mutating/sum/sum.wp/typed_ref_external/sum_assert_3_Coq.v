...
Goal
  ...
  ((i_0 <= n_0)%Z) ->
  ((n_0 <= 4294967295)%Z) ->
  (((2147483647 * i_0) <= (2147483647 * n_0))%Z) ->
  ((linked Malloc_0)) ->
  ((is_uint32 i_0%Z)) ->
  ((is_uint32 n_0%Z)) ->
  ((0 <= x_0)%Z) ->
  ...
  (((n_0 * x_1) <= 2147483647)%Z) ->
  (forall (i_1 : Z), ((i_1 < n_0)%Z) -> ((0 <= i_1)%Z) ->
   ((0 <= (Mint_0.[ (shift a_0 i_1%Z) ]))%Z)) ->
  (forall (i_1 : Z), let x_2 := (n_0 * (Mint_0.[ (shift a_0 i_1%Z) ]))%Z in
   ((i_1 < n_0)%Z) -> ((0 <= i_1)%Z) ->
   (((0 <= x_2)%Z) /\ ((x_2 <= 2147483647)%Z))) ->
  (((i_0 * x_1) <= 2147483647)%Z).

Proof.
  unfold is_uint32 in *.
  intros i n Malloc M a.
  intros.
  
  assert(X: i * (M .[ shift a i]) <= n * (M .[ shift a i])).
  auto with zarith.
  auto with zarith.
