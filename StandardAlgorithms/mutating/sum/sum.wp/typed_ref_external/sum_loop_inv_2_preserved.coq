(* ---------------------------------------------------------- *)
(* --- Preservation of Invariant (file sum.c, line 11)    --- *)
(* ---------------------------------------------------------- *)
...
Goal
  ...
  forall (a_0 : addr),
  let x_0 := (1%Z + i_0%Z)%Z in
  let x_1 := ((L_Sum Mint_0 a_0 i_0%Z))%Z in
  let a_1 := (shift a_0 i_0%Z) in
  let x_2 := (Mint_0.[ a_1 ])%Z in
  let x_3 := (x_1 + x_2)%Z in
  ((i_0 < n_0)%Z) ->
  ((0 <= i_0)%Z) ->
  ((i_0 <= 4294967294)%Z) ->
  ((i_0 <= n_0)%Z) ->
  ...
  ((x_3 <= 2147483647)%Z) ->
  ((is_sint32 x_3)) ->
  (forall (i_1 : Z), let x_4 := (Mint_0.[ (shift a_0 i_1%Z) ])%Z in
   ((i_1 < n_0)%Z) -> ((0 <= i_1)%Z) ->
   (((0 <= x_4)%Z) /\ ((x_4 <= 1000)%Z))) ->
  ((((L_Sum Mint_0 a_0 x_0)) = x_3)%Z)%Z.
