...
Theorem wp_goal :
  ...
  ((L_CountNotEqual_1_ t1 a i i1 i2) = (L_CountNotEqual_1_ t a i i1 i2)).
(* Why3 intros t t1 a i i1 i2 h1 h2 h3 h4. *)
Proof.
  Require Import Psatz.
  intros K L a m n v L32 K32 V32 Unchanged.
  revert Unchanged.
  ...
  }
  {
    ...
    }
    {
      ...
      assert(eq_neq: K((shift a (m+z)%Z)) = v \/ K((shift a (m+z)%Z)) <> v) by lia.
      destruct eq_neq as [eq|neq].
      - repeat rewrite Q_CountNotEqual_Miss; auto with zarith.
        + rewrite IHz; auto with zarith.
          apply Q_Unchanged_Shrink with (m:=m%Z) (n:=(1+(m+z))%Z); auto with zarith.
