...
(* Why3 assumption *)
Definition P_MinElement (Mint:addr -> Numbers.BinNums.Z) (a:addr)
    (n:Numbers.BinNums.Z) (min:Numbers.BinNums.Z) : Prop :=
  ((0%Z <= min)%Z /\ (min < n)%Z) /\
  P_LowerBound_1_ Mint a 0%Z n (Mint (shift a min)).
    ...
    }
    {
      ...
      - rewrite equal in *.
        ...
        assert(B: (L(shift a (L_HeapParent  z)) <= L(shift a 0))%Z).
        {
          ...
          - unfold L_HeapParent.
            replace (- (1) + z)%Z with (z-1)%Z by lia.
            enough ((z-1) ÷ 2 <= z-1)%Z; auto with zarith.
            apply int.ComputerDivision.Div_bound; lia.
        }
        auto with zarith.
     }
