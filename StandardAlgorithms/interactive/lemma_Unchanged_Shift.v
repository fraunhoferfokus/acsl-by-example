...
Theorem wp_goal :
  ...
  P_Unchanged_1_ t t1 (shift a i) i1 i2 ->
  P_Unchanged_1_ t t1 a (i + i1)%Z (i + i2)%Z.
(* Why3 intros t t1 a i i1 i2 h1 h2 h3. *)
Proof.
  Require Import Psatz.
  intros K L a p q r L32 K32 Unchanged.
  ...
  (* we must prove: for all indices k in [p+q, p+r), K and L agree *)
  intros k Hlow Hhigh.

  (* use Unchanged at index (k - p) for the shifted base (shift a p) *)
  specialize (Unchanged (k - p)%Z).
  assert (H1 : (q <= k - p)%Z) by lia.
  assert (H2 : (k - p < r)%Z) by lia.
  specialize (Unchanged H1 H2).

  (* rewrite the address using Hshift and then simplify p + (k - p) = k *)
  specialize (Hshift a p (k - p)%Z).
  rewrite Hshift in Unchanged.
  replace (p + (k - p))%Z with k in Unchanged by lia.
  auto.
Qed.
