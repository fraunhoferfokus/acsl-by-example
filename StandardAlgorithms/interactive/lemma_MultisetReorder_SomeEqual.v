...
Proof.
  ...
  intros L K a i n  x.
  subst x.
  intros nPos Lower Upper K32 L32 Reorder LAi32.
  ...
  apply Q_Count_SomeEqual; auto with zarith.
  rewrite Reorder; auto with zarith.
  rewrite <- Q_Count_Union with (m:=i); auto with zarith.
  assert(X: (0 <= L_Count_1_ L a 0 i (L(shift a i)))%Z) by 
    (apply Q_Count_Bounds; auto with zarith).

  enough(0 < L_Count_1_ L a i n (L(shift a i)))%Z by auto with zarith.
  rewrite <- Q_Count_Union with (m:=(1+i)%Z); auto with zarith.

  assert(Y:(0 <= L_Count_1_ L a (1 + i) n (L(shift a i)))%Z) by 
    (apply Q_Count_Bounds; auto with zarith).

  enough (0 < L_Count_1_ L a i (1 + i) (L(shift a i)))%Z by lia.
  rewrite <- Q_Count_Hit; auto with zarith.
  - rewrite Q_Count_Empty; auto with zarith.
  - now replace (-(1)+(1+i))%Z with i by lia.
Qed.
