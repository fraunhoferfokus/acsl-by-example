...
Theorem wp_goal :
  ...
  is_sint32 i2 -> (0%Z <= (L_IndexOfNotEqual t a i1 i2 i))%Z.
(* Why3 intros t a i i1 i2 h1 h2 h3 h4 h5. *)
Proof.
  Require Import Psatz.

  (*
    Rename the Why3-introduced variables to something readable:

      L   : memory (addr -> Z)
      a   : base address
      k   : index (rank among "not equal" elements)
      n   : upper bound
      v   : value to compare against

      nPos    : 0 < n
      kLower  : 0 <= k
      kUpper  : k < CountNotEqual(L, a, 0, n, v)
      L32     : memory values are sint32
      V32     : v is sint32
  *)
  intros L a k n v.
  intros nPos kLower kUpper L32 V32.

  (*
    We proceed by induction on k, using the nat-like recursion
    principle provided by Why3 (natlike_rec3).

    The induction hypothesis will establish the lower bound
    0 <= IndexOfNotEqual(...) for smaller indices.
  *)
  revert kUpper.
  ...
  (* ------------------------------------------------------------------ *)
  {
    ...
    (*
      Unfold IndexOfNotEqual at k = 0 using the "Next" lemma.
      This reduces the problem to the (-1)-case plus FindNotEqual.
    *)
    rewrite <- Q_IndexOfNotEqual_Next; auto with zarith.

    (* Normalize arithmetic artefacts introduced by Why3 *)
    replace (- (1) + 0)%Z with (-1)%Z by lia.

    (*
      For k = -1, IndexOfNotEqual is defined to be -1.
    *)
    rewrite Q_IndexOfNotEqual_Left; auto with zarith.

    (*
      The remaining term is FindNotEqual, whose result is always
      non-negative by Q_FindNotEqual_Lower.
    *)
    cbn.
  ...
  (* ------------------------------------------------------------------ *)
  (* Inductive step: k = z > 0                                           *)
  (* ------------------------------------------------------------------ *)
  {
    intros z zNN IHz.

    (*
      Replace Z.pred z by (z - 1) to match the form expected
      by the recursive lemmas.
    *)
    replace (Z.pred z) with (z - 1)%Z in * by lia.
    intros.

    (*
      Again unfold IndexOfNotEqual using the "Next" lemma.
      This expresses the current index in terms of the previous one.
    *)
    rewrite <- Q_IndexOfNotEqual_Next; auto with zarith.
    ...
    *)
    enough
      (0 <=
        L_FindNotEqual_1_
          L a
          (1 + L_IndexOfNotEqual L a n v (z - 1))
          n v)%Z
      by lia.
