...
(* Why3 assumption *)
Definition P_ArraySwap (Mint:addr -> Numbers.BinNums.Z)
    (Mint1:addr -> Numbers.BinNums.Z) (a:addr) (i:Numbers.BinNums.Z)
    (k:Numbers.BinNums.Z) (n:Numbers.BinNums.Z) : Prop :=
  ((((((((L_At Mint1 a i) = (L_At Mint a k)) /\
        ((L_At Mint1 a k) = (L_At Mint a i))) /\
       (0%Z <= i)%Z) /\
      (i < k)%Z) /\
     (k < n)%Z) /\
    P_Unchanged_1_ Mint Mint1 a 0%Z i) /\
   P_Unchanged_1_ Mint Mint1 a (1%Z + i)%Z k) /\
  P_Unchanged_1_ Mint Mint1 a (1%Z + k)%Z n.
...
Theorem wp_goal :
  ...
  is_sint32_chunk t1 -> is_sint32_chunk t -> P_ArraySwap t t1 a i i1 i2 ->
  P_MultisetReorder_1_ t t1 a i (1%Z + i1)%Z.
(* Why3 intros t t1 a i i1 i2 h1 h2 h3. *)
Proof.
  Require Import Psatz.
  intros L K a i k n.
  intros K32 L32 Swapped.
  ...
  rewrite <- Q_Count_Union with (m:=(1+i)%Z); auto with zarith.
  rewrite Z.add_comm.
  rewrite <- Q_Count_Union with (m:=k); auto with zarith.
  rewrite Q_Unchanged_MultisetReorder with (Mint:=L); auto with zarith.
  symmetry.
  rewrite <- Q_Count_Union with (m:=(1+i)%Z); auto with zarith.
  rewrite Z.add_comm.
  rewrite <- Q_Count_Union with (m:=k); auto with zarith.

  enough((L_Count_1_ K a i (1+i) v + L_Count_1_ K a k (1 + k) v)%Z =
         (L_Count_1_ L a i (1+i) v + L_Count_1_ L a k (1 + k) v)%Z) by auto with zarith.

  rewrite Z.add_comm.
  rewrite Q_Count_Single with (Mint:=L) (b:=a) (n:=i); auto with zarith.
  enough(L_Count_1_ K a i (1 + i) v = L_Count_1_ L a k (1 + k) v) by lia.  
  rewrite Q_Count_Single with (Mint:=L) (b:=a) (n:=k); auto with zarith.  
Qed.
