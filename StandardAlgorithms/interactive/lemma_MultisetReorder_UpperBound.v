...
Axiom Q_MultisetReorder_LowerBound :
  forall (Mint:addr -> Numbers.BinNums.Z) (Mint1:addr -> Numbers.BinNums.Z)
    (a:addr) (n:Numbers.BinNums.Z) (v:Numbers.BinNums.Z),
  (0%Z <= n)%Z -> is_sint32_chunk Mint -> is_sint32_chunk Mint1 ->
  is_sint32 v -> P_MultisetReorder_1_ Mint Mint1 a 0%Z n ->
  P_LowerBound_1_ Mint1 a 0%Z n v -> P_LowerBound_1_ Mint a 0%Z n v.

(* Why3 assumption *)
Definition P_SomeEqual_1_ (Mint:addr -> Numbers.BinNums.Z) (a:addr)
    (m:Numbers.BinNums.Z) (n:Numbers.BinNums.Z) (v:Numbers.BinNums.Z) : Prop :=
  exists i:Numbers.BinNums.Z,
  (((Mint (shift a i)) = v) /\ (m <= i)%Z) /\ (i < n)%Z.

(* Why3 assumption *)
Definition P_NoneEqual_1_ (Mint:addr -> Numbers.BinNums.Z) (a:addr)
    (m:Numbers.BinNums.Z) (n:Numbers.BinNums.Z) (v:Numbers.BinNums.Z) : Prop :=
  forall (i:Numbers.BinNums.Z), (m <= i)%Z -> (i < n)%Z ->
  ~ ((Mint (shift a i)) = v).

Axiom Q_NotSomeEqual_NoneEqual :
  forall (Mint:addr -> Numbers.BinNums.Z) (a:addr) (v:Numbers.BinNums.Z)
    (m:Numbers.BinNums.Z) (n:Numbers.BinNums.Z),
  is_sint32_chunk Mint -> is_sint32 v -> ~ P_SomeEqual_1_ Mint a m n v ->
  P_NoneEqual_1_ Mint a m n v.

Axiom Q_NoneEqual_NotSomeEqual :
  forall (Mint:addr -> Numbers.BinNums.Z) (a:addr) (v:Numbers.BinNums.Z)
    (m:Numbers.BinNums.Z) (n:Numbers.BinNums.Z),
  is_sint32_chunk Mint -> is_sint32 v -> P_NoneEqual_1_ Mint a m n v ->
  ~ P_SomeEqual_1_ Mint a m n v.

Axiom Q_MultisetReorder_SomeEqual :
  forall (Mint:addr -> Numbers.BinNums.Z) (Mint1:addr -> Numbers.BinNums.Z)
    (a:addr) (n:Numbers.BinNums.Z) (i:Numbers.BinNums.Z),
  let x := Mint (shift a i) in
  (0%Z < n)%Z -> (0%Z <= i)%Z -> (i < n)%Z -> is_sint32_chunk Mint ->
  is_sint32_chunk Mint1 -> P_MultisetReorder_1_ Mint Mint1 a 0%Z n ->
  is_sint32 x -> P_SomeEqual_1_ Mint1 a 0%Z n x.

Axiom Q_Count_SomeEqual :
  forall (Mint:addr -> Numbers.BinNums.Z) (a:addr) (v:Numbers.BinNums.Z)
    (m:Numbers.BinNums.Z) (n:Numbers.BinNums.Z),
  (0%Z <= m)%Z -> (m < n)%Z -> (0%Z < (L_Count_1_ Mint a m n v))%Z ->
  is_sint32_chunk Mint -> is_sint32 v -> P_SomeEqual_1_ Mint a m n v.
...
Theorem wp_goal :
  ...
  (0%Z <= i)%Z -> is_sint32_chunk t1 -> is_sint32_chunk t -> is_sint32 i1 ->
  P_MultisetReorder_1_ t t1 a 0%Z i -> P_UpperBound_1_ t1 a 0%Z i i1 ->
  P_UpperBound_1_ t a 0%Z i i1.
(* Why3 intros t t1 a i i1 h1 h2 h3 h4 h5 h6. *)
Proof.
  Require Import Psatz.
  intros L K a n v.
  intros nNN K32 L32 V32 Reorder K_Upper.
  assert(n_cases: (0 < n \/ 0 = n \/ 0 > n)%Z) by apply Ztrichotomy.
  destruct n_cases as [pos | non_pos].
  - unfold P_UpperBound_1_.
    intros.
    assert (X: P_SomeEqual_1_ K a 0 n (L(shift a i))%Z).
    + apply Q_MultisetReorder_SomeEqual; auto with zarith.
    + unfold P_SomeEqual_1_ in X.
      destruct X as [k [[A B] C]].
      rewrite <- A.
      now apply K_Upper.
  - unfold P_UpperBound_1_.
    lia.
