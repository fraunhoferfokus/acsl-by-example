...
Theorem wp_goal :
  ...
  is_sint32 i2 ->
  ((L_IndexOfNotEqual t a (1%Z + i1)%Z i2 i) =
   (L_IndexOfNotEqual t a i1 i2 i)).
(* Why3 intros t a i i1 i2 h1 h2 h3 h4 h5. *)
Proof.
  Require Import Psatz.
  intros L a k n v.
  intros nLower kLower kUpper L32 V32.
  revert kUpper.

  apply natlike_rec3 with (z:=k); auto with zarith.
  {
    intros.
    ...
    - enough (L_CountNotEqual_1_ L a 0 n v <= L_CountNotEqual_1_ L a 0 (1 + n) v)%Z; auto with zarith.
      apply Q_CountNotEqual_WeaklyIncreasing; auto with zarith.
  }
  {
    intros z zNN IHz.
    replace (Z.pred z) with (z-1)%Z in * by lia.
    intro zUpper.  (* this is the reverted kUpper for this step *)

    (* We will need zUpper also for length (1+n) *)
    assert (zUpper' : (z < L_CountNotEqual_1_ L a 0%Z (1%Z + n) v)%Z).
    { 
      eapply Z.lt_le_trans; [exact zUpper|].
      eapply Q_CountNotEqual_WeaklyIncreasing; auto with zarith.
    }

    (* Expand both IndexOfNotEqual calls using "Next" *)
    rewrite <- (Q_IndexOfNotEqual_Next L a v (1+n)%Z z); auto with zarith.
    rewrite <- (Q_IndexOfNotEqual_Next L a v n z); auto with zarith.
    replace (- (1) + z)%Z with (z-1)%Z by lia.

    (* Now both sides have the shape (1 + x) + FindNotEqual ... .
     We want to show the x’s are equal, and the FindNotEqual’s are equal. *)

     (* First: rewrite the recursive x for (1+n) using IH *)
     (* The x is IndexOfNotEqual ... (z-1).  IH should give equality for (z-1). *)
     assert (z1Upper :
       ((z-1) < L_CountNotEqual_1_ L a 0%Z n v)%Z) by lia.

    specialize (IHz z1Upper).  
    (* depending on natlike_rec3’s IH shape *)
    (* After this, use IHz to rewrite the recursive index inside the LHS *)
    rewrite IHz.

    (* Now the recursive index is the same on both sides; set it to x *)
    set (x := L_IndexOfNotEqual L a n v (z-1)).

    (* Reduce the goal to equality of FindNotEqual with upper bound n vs (1+n) *)
    (* i.e. show: L_FindNotEqual ... (1+n) = L_FindNotEqual ... n *)
    set (m := (1%Z + x)%Z).

    (* We need m <= n and (m + FindNotEqual m n) < n for Q_FindNotEqual_Hit *)
    assert (xUpper : (x < n)%Z).
    { (* uses Upper on index z-1 *)
     eapply Q_IndexOfNotEqual_Upper; auto with zarith.
    }

    assert (mLe : (m <= n)%Z) by (unfold m; lia).

    (* Obtain the strict "hit-before-n" inequality from Upper + Next at k=z *)
    assert (hitBefore : (m + L_FindNotEqual_1_ L a m n v < n)%Z).
    {
      (* From Next: Index(z) = m + FindNotEqual(m,n) *)
      assert(NextEq : 
        (m + L_FindNotEqual_1_ L a m n v)%Z =
      ...
      assert (Hit :
        L_FindNotEqual_1_ L a m n v =
        L_FindNotEqual_1_ L a m (1 + n) v).
      {
        (* We prove the equality by rewriting with Q_FindNotEqual_Hit. *)
        rewrite <- Q_FindNotEqual_Hit; auto with zarith.

        (* Remaining goal is the "hit-before-n" premise. *)
        rewrite NextEq.
        apply Q_IndexOfNotEqual_Upper; auto with zarith.
      }
      rewrite Hit.

      assert (UpZ: (L_IndexOfNotEqual L a n v z < n)%Z).
      {
        eapply Q_IndexOfNotEqual_Upper; auto with zarith.
      }
      lia.
    ...
    assert (FH :
      ...
      L_FindNotEqual_1_ L a m n v).
    {
      eapply Q_FindNotEqual_Hit; auto with zarith.
    }
    lia.
  }
Qed.
