...
Theorem wp_goal :
  ...
  P_Unchanged_1_ t t1 a i i1 ->
  ((L_FindNotEqual_1_ t1 a i i1 i2) = (L_FindNotEqual_1_ t a i i1 i2)).
(* Why3 intros t t1 a i i1 i2 h1 h2 h3 h4. *)
Proof.
  Require Import Psatz.
  intros K L a m n v L32 K32 V32.
  ...
  }
  {
    ...
    }
    {
      ...
      {
        apply Q_Unchanged_Shrink 
                with (n:=(1 + (m + z))%Z) (m:=m); auto with zarith.
      }
      ...
      assert(eq_neq: L(shift a (m+z)%Z) = v \/ L((shift a (m+z)%Z)) <> v) by lia.

      destruct found_notfound as [found|notfound]; auto with zarith.
      {
        destruct eq_neq as [eq|neq].
        {
          (* assert(K(shift a (m + z)) = v) by (rewrite H; auto with zarith). *)
      ...
      }
      {
        destruct eq_neq as [eq|neq].
        {
          (* assert(L(shift a (m + z)) = v) by (rewrite H; auto with zarith). *)
          enough(m + L_FindNotEqual_1_ K a m (1 + (m + z)) v =
                 m + L_FindNotEqual_1_ L a m (1 + (m + z)) v)%Z by lia.
          rewrite Q_FindNotEqual_MissMiss with (Mint:=K) (a:=a) (v:=v) (m:=m); auto with zarith.
          rewrite Q_FindNotEqual_MissMiss with (Mint:=L) (a:=a) (v:=v) (m:=m); auto with zarith.
          rewrite IHz; auto with zarith.
          rewrite <- eq.
        ...
        }
        {
          enough(m + L_FindNotEqual_1_ K a m (1 + (m + z)) v =
                 m + L_FindNotEqual_1_ L a m (1 + (m + z)) v)%Z by lia.
          rewrite Q_FindNotEqual_MissHit with (Mint:=K) (a:=a) (v:=v) (m:=m); auto with zarith.
          symmetry.
          rewrite Q_FindNotEqual_MissHit with (Mint:=L) (a:=a) (v:=v) (m:=m); auto with zarith.
          rewrite IHz; auto with zarith.
          rewrite <- U; auto with zarith.
        }
      }
