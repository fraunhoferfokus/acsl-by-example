...
Theorem wp_goal :
  ...
  (i <= i1)%Z -> (0%Z <= i)%Z -> is_sint32_chunk t ->
  P_WeaklyIncreasing_1_ t a i i1 -> P_Increasing_1_ t a i i1.
(* Why3 intros t a i i1 h1 h2 h3 h4. *)
Proof.
  ...
  }
  {
    ...
    {
      rewrite <- eq in *.
      assert(XX: (i < j - 1 \/ i = j - 1)%Z) by lia.
      destruct XX as [lt1|eq1].
      {
        assert(Y: (L(shift a (j-1)) <= L(shift a j))%Z).
        {
          replace j with (1 + (j-1))%Z by lia.
          replace ((1 + (j - 1) - 1))%Z with (j-1)%Z by auto with zarith.
          unfold P_WeaklyIncreasing_1_ in Weak.
          apply Weak; lia.
        }
        assert(Z: (L(shift a i) <= L(shift a (j-1)))%Z).
        {
          apply IHz; auto with zarith.
          unfold P_WeaklyIncreasing_1_.
          intros k. intros.
