...
Theorem wp_goal :
  ...
  ((i1 + (L_Find_1_ t a i1 i x))%Z = i) -> (i < i2)%Z -> (i1 <= i)%Z ->
  is_sint32_chunk t -> is_sint32 x -> ((i1 + (L_Find_1_ t a i1 i2 x))%Z = i).
(* Why3 intros t a i i1 i2 x h1 h2 h3 h4 h5. *)
Proof.
  Require Import Psatz.
  intros L a k m n ak Found kUpper kLower Chunk AK.
  pose (p := (n - m)%Z). 
