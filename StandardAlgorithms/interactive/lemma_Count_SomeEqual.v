...
Proof.
  ...
  {
    intros.
    rewrite Q_Count_Empty in Count; auto with zarith.
    lia.
  }
  {
    ...
    {
      rewrite <- Q_Count_Miss in IH; auto with zarith.
      replace (- (1) + (m + z))%Z with (m + z - 1)%Z in * by lia.
      ...
      + unfold P_SomeEqual_1_ .
        ...
        split; auto with zarith.
      + apply IHz; auto.
      + replace (- (1) + (m + z))%Z with (m + z - 1)%Z by lia.
        auto.
    }
  }
Qed.
