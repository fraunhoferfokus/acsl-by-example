...
Proof.
  ...
  - rewrite <- equal.
    assert(U: (m+L_Find_1_ t a m (m+z) ak <= m+z)%Z).
    + apply Q_Find_Upper; auto with zarith.
    + assert(hit_miss: (L_Find_1_ t a m (m+z) ak < z \/ L_Find_1_ t a m (m+z) ak = z)%Z) by lia.
