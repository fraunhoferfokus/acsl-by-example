...
Parameter L_InnerProduct:
  (addr -> Numbers.BinNums.Z) -> addr -> addr -> Numbers.BinNums.Z ->
  Numbers.BinNums.Z -> Numbers.BinNums.Z.

Axiom L_InnerProduct_def :
  forall (Mint:addr -> Numbers.BinNums.Z) (a:addr) (b:addr)
    (n:Numbers.BinNums.Z) (init:Numbers.BinNums.Z),
  let x := ((-1%Z)%Z + n)%Z in
  ((n <= 0%Z)%Z -> ((L_InnerProduct Mint a b n init) = init)) /\
  (~ (n <= 0%Z)%Z ->
   ((L_InnerProduct Mint a b n init) =
    ((L_InnerProduct Mint a b x init) +
     ((Mint (shift a x)) * (Mint (shift b x)))%Z)%Z)).

Axiom Q_InnerProduct_Next :
  forall (Mint:addr -> Numbers.BinNums.Z) (a:addr) (b:addr)
    (init:Numbers.BinNums.Z) (n:Numbers.BinNums.Z),
  let x := Mint (shift a n) in
  let x1 := Mint (shift b n) in
  (0%Z <= n)%Z -> is_sint32_chunk Mint -> is_sint32 init -> is_sint32 x ->
  is_sint32 x1 ->
  (((L_InnerProduct Mint a b n init) + (x * x1)%Z)%Z =
   (L_InnerProduct Mint a b (1%Z + n)%Z init)).

Axiom Q_InnerProduct_Init :
  forall (Mint:addr -> Numbers.BinNums.Z) (a:addr) (b:addr)
    (init:Numbers.BinNums.Z) (n:Numbers.BinNums.Z),
  (n <= 0%Z)%Z -> is_sint32_chunk Mint -> is_sint32 init ->
  ((L_InnerProduct Mint a b n init) = init).
...
Theorem wp_goal :
  ...
  P_Unchanged_1_ t t1 a1 0%Z i1 -> P_Unchanged_1_ t t1 a 0%Z i1 ->
  ((L_InnerProduct t1 a a1 i1 i) = (L_InnerProduct t a a1 i1 i)).
(* Why3 intros t t1 a a1 i i1 h1 h2 h3 h4 h5. *)
Proof.
  Require Import Psatz.
  intros K L a b init n.
  intros LChunk KChunk VChunk.
  assert (X: (n <= 0 \/ n > 0)%Z) by lia.
  destruct X as [lez|gtz].
  {
    intros.
    repeat rewrite Q_InnerProduct_Init; auto with zarith.
  }
  ...
  }
  {
      ...
      intros a_Unchanged b_Unchanged.
      assert(Shrink_a: P_Unchanged_1_ K L a 0 z).
      {
        apply Q_Unchanged_Shrink with
        (m:=0%Z) (n:=(1+z)%Z); auto with zarith.
      }
      assert(Shrink_b: P_Unchanged_1_ K L b 0 z).
      {
        apply Q_Unchanged_Shrink with
        (m:=0%Z) (n:=(1+z)%Z); auto with zarith.
      }
      rewrite <- Q_InnerProduct_Next; auto with zarith.
      symmetry.
      rewrite <- Q_InnerProduct_Next; auto with zarith.
      rewrite IHz; auto with zarith.
      rewrite a_Unchanged; auto with zarith.
      rewrite b_Unchanged; auto with zarith.
   }
Qed.
