...
void test_remove_copy(const std::vector<value_type>& a, value_type v)
{
  ...
  const size_type kept = remove_copy(a.data(), a.size(), b.data(), v);
...
int main(int, char**)
{
  for (const auto& a : test_arrays()) {
    test_remove_copy(a, present_value());
    test_remove_copy(a, absent_value());        // nothing is removed

    for (size_type i = 0; i < a.size(); ++i) {
      test_remove_copy(a, a[i]);
    }
  }

  test_remove_copy({3, 3, 3}, 3);               // everything is removed
