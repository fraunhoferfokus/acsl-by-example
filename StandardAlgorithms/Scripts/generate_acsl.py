...
re_function  = re.compile(r"""logic[\wℤ𝔹\s\*]+\s+([a-zA-Z_]\w*)""", re.X)
...
def generate_acsl(fname):
	...
	name   = bname.replace('.acsl', '').replace('_', '')
	...
	acsl = set()
	acsl = sorted(acsl.union(axiom, axiomatic, function, inductive, lemma, predicate))
	...
	if (len(axiomatic) == 1):
		...
		c += '\\' + axiomatic[0].replace('_', '')
		c += '\n'
	elif (len(inductive) == 1):
		c += 'Inductive definition '
		c += '\\' + inductive[0].replace('_', '')
		c += '\n'
	else:
		c += 'File \inl{' + bname + '}'
		c += '\\label{logic:' + bname + '}\n'
