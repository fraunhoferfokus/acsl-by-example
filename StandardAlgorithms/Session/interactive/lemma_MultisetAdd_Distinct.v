...
Theorem wp_goal :
  ...
  ~ (x = x1) -> (0%Z <= i)%Z -> (i < i1)%Z -> is_sint32_chunk t1 ->
  is_sint32_chunk t -> is_sint32 x -> is_sint32 x1 ->
  P_MultisetReorder_1_ t t1 a 0%Z i ->
  P_MultisetReorder_1_ t t1 a (1%Z + i)%Z i1 -> P_MultisetAdd t t1 a i1 x1.
(* Why3 intros t t1 a i i1 x x1 h1 h2 h3 h4 h5 h6 h7 h8 h9. *)
Proof.
  Require Import Psatz.
  intros L K a i n Kai Lai.
  unfold L_At in *.
  intros distinct iNN iLess KChunk LChunk KaiChunk LaiChunk.
  intros Reorder1 Reorder2.
  unfold P_MultisetAdd.
  symmetry.
  rewrite <- Q_Count_Cut with (m:=i); auto with zarith.
  rewrite <- Reorder1; auto with zarith.
  rewrite <- Reorder2; auto with zarith.
  assert(X: (- (1) + (1 + i) = i)%Z) by lia.
  replace (L_Count_1_ L a i (1 + i) Lai)%Z with 1%Z.
  - enough (L_Count_1_ K a 0 n Lai = L_Count_1_ K a (1 + i) n Lai + L_Count_1_ K a 0 i Lai)%Z by lia.
    rewrite <- Q_Count_Cut with (m:=i); auto with zarith.
    replace (L_Count_1_ K a i (1 + i) Lai)%Z with 0%Z; auto with zarith.
