...
(* Why3 intros t t1 a i i1 h1 h2 h3 h4. *)
Proof.
  Require Import Psatz.
  Require Import ZArith.
  intros K L a init n L32 K32 v32.

  assert (nonpos_pos: (n <= 0 \/ n > 0)%Z) by lia.
  destruct nonpos_pos as [nonpos|pos].
  {
    intros Unchanged.
    repeat rewrite Q_Accumulate_Init; auto with zarith.
  }
  {
    (* now induction *)
    ...
    }
    {
      ...
      intros Unchanged.
      assert(Shrink: P_Unchanged_1_ K L a 0 z).
      {
        apply Q_Unchanged_Shrink with
        (m:=0%Z) (n:=(1+z)%Z); auto with zarith.
      }
      rewrite <- Q_Accumulate_Next; auto with zarith.
      symmetry.
      rewrite <- Q_Accumulate_Next; auto with zarith.
      rewrite IHz; auto with zarith.
      rewrite Unchanged; lia.
    }
  }
Qed.
