...
(* Why3 assumption *)
Definition P_Heap (Mint:addr -> Numbers.BinNums.Z) (a:addr)
    (n:Numbers.BinNums.Z) : Prop :=
  forall (i:Numbers.BinNums.Z), (0%Z < i)%Z -> (i < n)%Z ->
  ((Mint (shift a i)) <= (Mint (shift a (L_HeapParent i))))%Z.
...
(* Why3 assumption *)
Definition P_HeapCompatible (Mint:addr -> Numbers.BinNums.Z) (a:addr)
    (n:Numbers.BinNums.Z) (m:Numbers.BinNums.Z) (v:Numbers.BinNums.Z) : Prop :=
  let x := L_HeapParent m in
  let x1 := L_HeapLeft m in
  let x2 := L_HeapRight m in
  ((((0%Z <= m)%Z /\ (m < n)%Z) /\
    ((0%Z <= x)%Z -> (v <= (Mint (shift a x)))%Z)) /\
   ((x1 < n)%Z -> ((Mint (shift a x1)) <= v)%Z)) /\
  ((x2 < n)%Z -> ((Mint (shift a x2)) <= v)%Z).
...
(* Why3 assumption *)
Definition P_ArrayUpdate (Mint:addr -> Numbers.BinNums.Z)
    (Mint1:addr -> Numbers.BinNums.Z) (a:addr) (n:Numbers.BinNums.Z)
    (i:Numbers.BinNums.Z) (v:Numbers.BinNums.Z) : Prop :=
  ((((((L_At Mint a i) = v) /\ ~ ((L_At Mint1 a i) = v)) /\ (0%Z <= i)%Z) /\
    (i < n)%Z) /\
   P_Unchanged_1_ Mint Mint1 a 0%Z i) /\
  P_Unchanged_1_ Mint Mint1 a (1%Z + i)%Z n.

Axiom Q_HeapCompatible_Update :
  forall (Mint:addr -> Numbers.BinNums.Z) (Mint1:addr -> Numbers.BinNums.Z)
    (a:addr) (v:Numbers.BinNums.Z) (m:Numbers.BinNums.Z)
    (n:Numbers.BinNums.Z),
  (0%Z <= m)%Z -> (m < n)%Z -> is_sint32_chunk Mint ->
  is_sint32_chunk Mint1 -> is_sint32 v -> P_Heap Mint1 a n ->
  P_HeapCompatible Mint1 a n m v -> P_ArrayUpdate Mint Mint1 a n m v ->
  P_Heap Mint a n.

Axiom Q_Heap_Unchanged :
  forall (Mint:addr -> Numbers.BinNums.Z) (Mint1:addr -> Numbers.BinNums.Z)
    (a:addr) (n:Numbers.BinNums.Z),
  (0%Z <= n)%Z -> is_sint32_chunk Mint -> is_sint32_chunk Mint1 ->
  P_Heap Mint1 a n -> P_Unchanged_1_ Mint Mint1 a 0%Z n -> P_Heap Mint a n.

Axiom Q_Heap_Shrink :
  forall (Mint:addr -> Numbers.BinNums.Z) (a:addr) (m:Numbers.BinNums.Z)
    (n:Numbers.BinNums.Z),
  (0%Z <= m)%Z -> (m <= n)%Z -> is_sint32_chunk Mint -> P_Heap Mint a n ->
  P_Heap Mint a m.
...
(* Why3 assumption *)
Definition P_MaxElement (Mint:addr -> Numbers.BinNums.Z) (a:addr)
    (n:Numbers.BinNums.Z) (max:Numbers.BinNums.Z) : Prop :=
  ((0%Z <= max)%Z /\ (max < n)%Z) /\
  P_UpperBound_1_ Mint a 0%Z n (Mint (shift a max)).

Axiom Q_Heap_Maximum :
  forall (Mint:addr -> Numbers.BinNums.Z) (a:addr) (n:Numbers.BinNums.Z),
  (0%Z < n)%Z -> is_sint32_chunk Mint -> P_Heap Mint a n ->
  P_MaxElement Mint a n 0%Z.

Axiom Q_ArrayUpdate_UpperBound :
  forall (Mint:addr -> Numbers.BinNums.Z) (Mint1:addr -> Numbers.BinNums.Z)
    (a:addr) (v:Numbers.BinNums.Z) (w:Numbers.BinNums.Z)
    (n:Numbers.BinNums.Z) (i:Numbers.BinNums.Z),
  (v <= w)%Z -> is_sint32_chunk Mint -> is_sint32_chunk Mint1 ->
  is_sint32 v -> is_sint32 w -> P_UpperBound_1_ Mint1 a 0%Z n w ->
  P_ArrayUpdate Mint Mint1 a n i v -> P_UpperBound_1_ Mint a 0%Z n w.

Axiom Q_ArrayUpdate_Shrink :
  forall (Mint:addr -> Numbers.BinNums.Z) (Mint1:addr -> Numbers.BinNums.Z)
    (a:addr) (v:Numbers.BinNums.Z) (n:Numbers.BinNums.Z)
    (i:Numbers.BinNums.Z),
  (0%Z <= i)%Z -> ((2%Z + i)%Z <= n)%Z -> is_sint32_chunk Mint ->
  is_sint32_chunk Mint1 -> is_sint32 v -> P_ArrayUpdate Mint Mint1 a n i v ->
  P_ArrayUpdate Mint Mint1 a ((-1%Z)%Z + n)%Z i v.
...
Theorem wp_goal :
  ...
  (0%Z <= i1)%Z -> (0%Z <= i)%Z -> is_sint32_chunk t1 -> is_sint32_chunk t ->
  P_MultisetReorder_1_ t t1 (shift a i1) 0%Z i ->
  P_MultisetReorder_1_ t t1 a i1 (i + i1)%Z.
Proof.
  Require Import Psatz.
  intros t t1 a i i1 h1 h2 t1_32 t_32 Reorder.

  unfold P_MultisetReorder_1_ in *.
  intros v Hv.

  (* the shifted range, in the coordinates of the whole array, at each label *)
  assert (HS1 : (L_Count_1_ t1 (shift a i1) 0%Z i v)
                = (L_Count_1_ t1 a i1 (i + i1)%Z v)).
  {
    assert (HS := Q_Count_Shift t1 a v i1 i).
    replace (i1 + i)%Z with (i + i1)%Z in HS by lia.
    apply HS; [lia | lia | assumption | assumption].
  }

  assert (HS2 : (L_Count_1_ t (shift a i1) 0%Z i v)
                = (L_Count_1_ t a i1 (i + i1)%Z v)).
  {
    assert (HS := Q_Count_Shift t a v i1 i).
    replace (i1 + i)%Z with (i + i1)%Z in HS by lia.
    apply HS; [lia | lia | assumption | assumption].
  }
