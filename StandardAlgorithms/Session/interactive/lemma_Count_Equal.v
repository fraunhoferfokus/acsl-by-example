...
Theorem wp_goal :
  ...
  is_sint32 i3 -> P_Equal_4_ t t1 a i i1 i2 ->
  ((L_Count_1_ t a i2 ((i1 + i2)%Z + ((-1%Z)%Z * i)%Z)%Z i3) =
   (L_Count_1_ t1 a i i1 i3)).
(* Why3 intros t t1 a i i1 i2 i3 h1 h2 h3 h4 h5 h6. *)
Proof.
  ...
  }
  {
    ...
    rewrite Q_Count_Single with (n:=(p+z)%Z) (b:=a) (Mint:=L); auto with zarith.
    rewrite <- shift_shift.
    unfold P_Equal_4_ in Equal.
