...
Theorem wp_goal :
  ...
  ((L_Count_1_ t1 a i i1 i2) = (L_Count_1_ t a i i1 i2)).
(* Why3 intros t t1 a i i1 i2 h1 h2 h3 h4. *)
Proof.
  Require Import Psatz.
  intros L K a m n v K32 L32 V32 Unchanged.
  ...
  }
  {
    ...
    }
    {
      ...
      rewrite <- Q_Count_One; auto with zarith.
      rewrite <- IHz. 
      enough((L_Count_1_ L a (m + z) (1 + (m + z)) v)
            = L_Count_1_ K a (m + z) (1 + (m + z)) v) by lia.
      - rewrite <- Q_Count_Single 
          with (Mint:=K) (Mint1:= L) (a:=a) (m:=(m+z)%Z) ; auto with zarith.
        rewrite Unchanged; lia.
      - apply Q_Unchanged_Shrink with (m:=m) (n:=(1+(m+z))%Z); auto with zarith.
    }
  }
Qed.
