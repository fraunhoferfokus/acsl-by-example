...
(* Why3 goal *)
Theorem wp_goal :
  forall (i:Numbers.BinNums.Z), (0%Z <= i)%Z -> P_HeapAncestor 0%Z i.
Proof.
  ...
  refine ((Z_lt_induction
    (fun c => (0 <= c)%Z -> P_HeapAncestor 0%Z c)
    _) i Hi Hi).
    
  intros c IH Hc_nonneg.
  destruct (Z.eq_dec c 0%Z) as [-> | Hc_not_zero].
  
  - constructor. lia. 

  - apply Q_P_HeapAncestor_HeapAncestor_Step; [lia |].
    pose proof (Q_Heap_ParentBounds c ltac:(lia)) as Hparent_bounds.
    apply IH; lia.
