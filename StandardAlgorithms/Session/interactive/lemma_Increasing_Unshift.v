...
Theorem wp_goal :
  ...
  (i1 <= i2)%Z -> (0%Z <= i1)%Z -> (0%Z <= i)%Z -> is_sint32_chunk t ->
  P_Increasing_1_ t (shift a i) i1 i2 ->
  P_Increasing_1_ t a (i + i1)%Z (i + i2)%Z.
(* Why3 intros t a i i1 i2 h1 h2 h3 h4 h5. *)
Proof.
  Require Import Psatz.
  intros t a i i1 i2 h1 h2 h3 t32 Increasing.

  (* inline version of shift_shift *)
  assert (Hshift :
  ...
  (* we must prove: a[j] <= a[k] for all i+i1 <= j < k < i+i2 *)
  intros j k Hjk Hlow Hhigh.

  (* use the hypothesis at the shifted indices j-i and k-i *)
  specialize (Increasing (j - i)%Z (k - i)%Z).
  assert (H1 : (j - i < k - i)%Z) by lia.
  assert (H2 : (i1 <= j - i)%Z) by lia.
  assert (H3 : (k - i < i2)%Z) by lia.
  specialize (Increasing H1 H2 H3).

  (* rewrite the addresses and simplify i + (j - i) = j *)
  rewrite (Hshift a i (j - i)%Z) in Increasing.
  rewrite (Hshift a i (k - i)%Z) in Increasing.
  replace (i + (j - i))%Z with j in Increasing by lia.
  replace (i + (k - i))%Z with k in Increasing by lia.
  auto.
Qed.
