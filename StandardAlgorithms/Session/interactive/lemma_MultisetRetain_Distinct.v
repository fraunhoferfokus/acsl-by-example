...
(* Why3 assumption *)
Definition P_MultisetAdd (Mint:addr -> Numbers.BinNums.Z)
    (Mint1:addr -> Numbers.BinNums.Z) (a:addr) (n:Numbers.BinNums.Z)
    (v:Numbers.BinNums.Z) : Prop :=
  ((1%Z + (L_Count_1_ Mint1 a 0%Z n v))%Z = (L_Count_1_ Mint a 0%Z n v)).

Axiom Q_MultisetMinus_Distinct :
  forall (Mint:addr -> Numbers.BinNums.Z) (Mint1:addr -> Numbers.BinNums.Z)
    (a:addr) (v:Numbers.BinNums.Z) (m:Numbers.BinNums.Z)
    (n:Numbers.BinNums.Z),
  let x := L_At Mint a m in
  let x1 := L_At Mint1 a m in
  ~ (x = v) -> (x1 = v) -> (0%Z <= m)%Z -> (m < n)%Z ->
  is_sint32_chunk Mint -> is_sint32_chunk Mint1 -> is_sint32 v ->
  is_sint32 x -> is_sint32 x1 -> P_MultisetReorder_1_ Mint Mint1 a 0%Z m ->
  P_MultisetReorder_1_ Mint Mint1 a (1%Z + m)%Z n ->
  P_MultisetAdd Mint1 Mint a n v.

Axiom Q_MultisetAdd_Distinct :
  forall (Mint:addr -> Numbers.BinNums.Z) (Mint1:addr -> Numbers.BinNums.Z)
    (a:addr) (v:Numbers.BinNums.Z) (m:Numbers.BinNums.Z)
    (n:Numbers.BinNums.Z),
  let x := L_At Mint a m in
  ...
  is_sint32 x -> is_sint32 x1 -> P_MultisetReorder_1_ Mint Mint1 a 0%Z m ->
  P_MultisetReorder_1_ Mint Mint1 a (1%Z + m)%Z n ->
  P_MultisetAdd Mint Mint1 a n v.
...
(* Why3 assumption *)
Definition P_MultisetRetain (Mint:addr -> Numbers.BinNums.Z)
    (Mint1:addr -> Numbers.BinNums.Z) (a:addr) (n:Numbers.BinNums.Z)
    (v:Numbers.BinNums.Z) : Prop :=
  ((L_Count_1_ Mint1 a 0%Z n v) = (L_Count_1_ Mint a 0%Z n v)).
...
Theorem wp_goal :
  ...
  let x := L_At t1 a i in
  let x1 := L_At t a i in
  ~ (x = i2) -> ~ (x1 = i2) -> (0%Z <= i)%Z -> (i < i1)%Z ->
  is_sint32_chunk t1 -> is_sint32_chunk t -> is_sint32 i2 -> is_sint32 x ->
  is_sint32 x1 -> P_MultisetReorder_1_ t t1 a 0%Z i ->
  P_MultisetReorder_1_ t t1 a (1%Z + i)%Z i1 -> P_MultisetRetain t t1 a i1 i2.
(* Why3 intros t t1 a i i1 i2 x x1 h1 h2 h3 h4 h5 h6 h7 h8 h9 h10 h11. *)
Proof.
  ...
  rewrite <- Q_Count_Cut with (m:=m); auto with zarith.
  ...
  - rewrite Q_Count_Empty; auto.
    rewrite <- Q_Count_Miss; auto with zarith.
    + replace (- (1) + (1 + m))%Z with m by lia.
