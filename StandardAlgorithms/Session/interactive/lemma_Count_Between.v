...
Theorem wp_goal :
  ...
  is_sint32 i3 -> P_Equal_3_ t t1 a i i1 a1 i2 ->
  ((L_Count_1_ t a1 i2 ((i1 + i2)%Z + ((-1%Z)%Z * i)%Z)%Z i3) =
   (L_Count_1_ t1 a i i1 i3)).
Proof.
  Require Import Psatz.
  intros L K a b m n p v mUpper mLower L32 K32 V32 Equal.
  remember (n-m)%Z as k.
  ...
  }
  {
    ...
    assert(Y: P_Equal_3_ L K a m (m + z) b p).
    {
    ...
    enough(L_Count_1_ K a (m + z) (1 + (m + z)) v = L_Count_1_ L b (p + z) (1 + (p + z)) v)%Z by lia.
    apply Q_Count_Single with (Mint:=L) (Mint1:=K); auto with zarith.
    unfold P_Equal_3_ in Equal.
    replace (m+z)%Z with (z+m)%Z by lia.
    replace (p+z)%Z with (z+p)%Z by lia.
    apply Equal; lia.
  }
Qed.
